...
const ProductInformationSupplementList = ({ samples, setActiveTab, setLayoutMode: setGlobalLayoutMode }) => {
  ...
  // 2. 定义所有字段及其属性 (isRequired 严格按照您的需求)
  const allColumns = [
    // 样品信息 (基础项，始终展示)
    ...
    { group: 'base', key: 'purchaser', label: '采购员', width: 'w-[100px]', sticky: true, offset: '268px', type: 'purchaser' },
    { group: 'base', key: 'status', label: '当前状态', width: 'w-[100px]', sticky: true, offset: '368px', type: 'status' },
    
    // 基本属性 (17个字段中 12 个必填)
    { group: 'basic', key: 'brand', label: '公司品牌', width: 'w-[100px]', isRequired: true },
  ...
  // 3. 核心逻辑：根据当前视图过滤列
  const displayColumns = viewMode === '必填字段视图' 
    ? allColumns.filter(c => c.isRequired || c.group === 'base') 
    : allColumns;

  // 4. 核心逻辑：根据过滤后的列重新计算分组
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-gray-950 animate-in fade-in duration-500">
      ...
      {/* 顶部工具栏 */}
      <div className="px-6 py-2.5 border-b border-slate-200 dark:border-gray-800 flex items-center justify-between bg-slate-50/30 dark:bg-gray-900">
        <div className="flex items-center space-x-4 text-[11px]">
           <div className="flex items-center space-x-2 font-bold text-slate-500">
              <Filter size={14} />
              <span>视图设置:</span>
              <select 
                value={viewMode}
                onChange={(e) => setViewMode(e.target.value)}
                className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded px-2 py-0.5 text-blue-600 outline-none transition-all shadow-sm focus:ring-2 ring-blue-500/20"
              >
           ...
           {viewMode === '必填字段视图' && (
             <span className="flex items-center text-amber-600 font-bold bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded animate-pulse">
        ...
        <div className="flex items-center space-x-2 text-[11px] text-slate-400 font-mono">
          ...
          <span>当前列数: {displayColumns.length}</span>
        </div>
      </div>

      {/* 滚动容器 */}
      <div className="flex-1 overflow-auto no-scrollbar">
        <table className="border-separate border-spacing-0 text-left w-max">
          <thead className="sticky top-0 z-30">
            {/* 第一层：分组标题 */}
            <tr className="bg-slate-50 dark:bg-gray-800">
              <th rowSpan={2} className="px-6 py-3 w-12 border-b border-slate-200 dark:border-gray-700 sticky left-0 z-50 bg-slate-50 dark:bg-gray-800 border-r border-slate-200 dark:border-gray-700 shadow-[1px_0_0_0_#e2e8f0]">
              ...
              {displayColumns.map(col => (
                ...
                >
                  <div className="flex items-center justify-center space-x-1 group">
                    <span>{col.label}</span>
                    <ArrowUpDown size={10} className="text-slate-300 opacity-0 group-hover:opacity-100 cursor-pointer" />
                  </div>
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
            {samples.map((sample) => (
              <tr key={sample.id} className="hover:bg-blue-50/20 dark:hover:bg-blue-900/5 transition-colors group">
                {/* 动态渲染所有单元格 */}
                <td className="px-6 py-4 sticky left-0 z-20 bg-white dark:bg-gray-950 group-hover:bg-blue-50/50 dark:group-hover:bg-blue-900/10 transition-colors border-b border-r border-slate-100 dark:border-gray-800 text-center shadow-[1px_0_0_0_#f1f5f9]">
                  <input type="checkbox" className="rounded border-slate-300" />
                </td>
                ...
                {/* 固定操作列 */}
                <td className="px-6 py-4 text-right sticky right-0 z-20 bg-white dark:bg-gray-950 group-hover:bg-blue-50/50 dark:group-hover:bg-blue-900/10 transition-colors shadow-[-1px_0_0_0_#f1f5f9] border-b border-slate-100 dark:border-gray-800">
                  <div className="flex items-center justify-end space-x-1.5">
                    <button onClick={() => handleEdit(sample.id)} className="p-1.5 bg-blue-50 text-blue-600 rounded-md hover:bg-blue-100 transition-colors"><Edit3 size={14} /></button>
                    <button className="p-1.5 bg-slate-50 text-slate-500 rounded-md hover:bg-slate-100 transition-colors"><Copy size={14} /></button>
                    <button className="p-1.5 bg-red-50 text-red-500 rounded-md hover:bg-red-100 transition-colors"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {/* 底部汇总 */}
      <div className="px-6 py-3 border-t border-slate-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex items-center justify-between">
         <div className="flex items-center space-x-4">
            <button className="flex items-center space-x-1.5 text-[11px] font-bold text-blue-600 hover:bg-blue-50 px-3 py-1 rounded-md transition-all">
               <ShieldCheck size={14} />
               <span>批量应用属性</span>
            </button>
         </div>
...
const DynamicCell = ({ col, sample, onEdit }) => {
  ...
  const renderContent = () => {
    switch (col.type) {
      case 'image':
        return (
          <div className="w-10 h-10 mx-auto bg-slate-50 dark:bg-gray-800 rounded border border-slate-200 dark:border-gray-700 overflow-hidden flex items-center justify-center">
        ...
        return (
          <div className="flex justify-center">
            {sample.status === 'completed' ? (
              <div className="flex items-center space-x-1.5 text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100"><CheckCircle2 size={11} /><span className="text-[10px] font-black">DONE</span></div>
            ) : (
              <div className="flex items-center space-x-1.5 text-orange-600 bg-orange-50 px-2 py-0.5 rounded border border-orange-100"><AlertCircle size={11} /><span className="text-[10px] font-black">TODO</span></div>
            )}
          </div>
        );
      case 'purchaser':
      ...
      case 'button':
        return <button className="text-emerald-600 font-bold text-[11px] hover:underline">查看</button>;
      case 'link':
  ...
  return (
    <td className={commonClasses} style={style}>
      {renderContent()}
    </td>
  );
};
