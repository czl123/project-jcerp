...
const NewMaterialModal = ({ onClose }) => {
  ...
  const isManualScrolling = useRef(false); // 状态锁：防止手动点击跳转时触发 handleScroll 冲突

  // 核心逻辑：滚动监听
  const handleScroll = () => {
    if (isManualScrolling.current) return; // 如果正在执行手动跳转，不执行监听逻辑
    
    const container = scrollContainerRef.current;
    if (!container) return;

    const scrollPos = container.scrollTop + 120; // 增加偏移缓冲区
  ...
  const scrollToSection = (tab) => {
    ...
    if (sectionRefs[tab].current) {
      isManualScrolling.current = true; // 加锁
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-2 text-slate-800 antialiased font-sans text-[12px]">
      <div className="bg-white w-[1560px] max-w-full h-[96vh] rounded-xl shadow-2xl flex flex-col overflow-hidden border border-slate-200">
        
        {/* 顶部标题 */}
        <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 shrink-0 bg-white">
          <div className="flex items-center gap-2">
            <div className="w-1 h-4 bg-blue-600 rounded-full"></div>
            <h2 className="text-[13px] font-black text-slate-700">新建配件物料 <span className="text-slate-300 font-normal ml-2">COMPACT PRO</span></h2>
          </div>
          <button onClick={onClose} className="p-1 hover:bg-slate-50 rounded-md transition-colors"><X size={18} className="text-slate-400" /></button>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-[180px] shrink-0 border-r border-slate-100 flex flex-col bg-slate-50/20">
            <div className="p-4 border-b border-slate-100 flex flex-col items-center">
               ...
               <div className="text-[11px] font-bold text-slate-700 truncate w-32 text-center">{basicInfo.name || '未命名配件'}</div>
            </div>
            <div className="flex-1 p-1 space-y-0.5">
              {[
                ...
                <div key={tab.name} onClick={() => scrollToSection(tab.name)} className={`px-2.5 py-2 rounded-lg transition-all cursor-pointer flex items-center justify-between ${activeTab === tab.name ? 'bg-white text-blue-600 font-bold shadow-sm border border-slate-100' : 'text-slate-500 hover:bg-white/60'}`}>
                  <div className="flex items-center space-x-2">{tab.icon}<span className="text-[11px]">{tab.name}</span></div>
                  {tab.info > 0 && <span className="text-[9px] bg-blue-50 text-blue-500 px-1.5 py-0.5 rounded-md font-bold">{tab.info}</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          {/* 右侧主内容区 */}
          <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-4 space-y-8 scroll-smooth no-scrollbar">
            
            {/* 1. 基本信息区块 */}
            <div ref={sectionRefs['基本信息']} className="space-y-4">
              <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm pb-2 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center space-x-2 text-slate-800 font-bold text-[13px]"><Info size={14} className="text-blue-500" /><span>基本信息</span></div>
                <div className="flex items-center gap-2">
                   <span className="text-[10px] text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-100 flex items-center gap-1">
                      ...
                      <input 
                        className="w-24 bg-transparent outline-none border-none text-blue-600 font-bold placeholder:font-normal" 
                        placeholder="回车填充" 
                        value={citeTarget}
              ...
              <div className="bg-slate-50/30 p-4 rounded-xl border border-slate-100 space-y-6">
                 <div className="grid grid-cols-3 gap-x-6 gap-y-3">
                    <div className="col-span-3 flex items-center gap-2 text-[11px] font-bold text-blue-600 mb-1"><div className="w-1 h-3 bg-blue-500 rounded-full"></div> 同款属性 Identical</div>
                    ...
                    <div className="col-span-3 flex items-center gap-2 text-[11px] font-bold text-orange-600 mb-1"><div className="w-1 h-3 bg-orange-500 rounded-full"></div> 关键属性 Key Attrs</div>
                    ...
                    <div className="grid grid-cols-4 gap-x-4 gap-y-3 pt-4 border-t border-slate-50">
                    <div className="col-span-4 flex items-center gap-2 text-[11px] font-bold text-slate-500 mb-1 uppercase"><div className="w-1 h-3 bg-slate-300 rounded-full"></div> 一般属性 General</div>
                 ...
                 <div className="grid grid-cols-2 gap-x-8 gap-y-3 pt-4 border-t border-slate-50">
                    <div className="col-span-2 flex items-center gap-2 text-[11px] font-bold text-emerald-600 mb-1"><div className="w-1 h-3 bg-emerald-500 rounded-full"></div> 补充说明 Remarks</div>
            ...
            <div ref={sectionRefs['关联主物料']} className="space-y-3">
               <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm pb-2 border-b border-slate-100 flex items-center space-x-2 text-slate-800 font-bold text-[13px]"><Link2 size={14} className="text-blue-500" /><span>关联主物料</span></div>
               <div className="flex gap-2 px-2">
                  <div className="flex-1 relative flex items-center">
                     <Search size={13} className="absolute left-2.5 text-slate-400" />
                     <input className="w-full bg-slate-50 border border-slate-200 rounded pl-8 pr-3 h-8 text-[11px] outline-none focus:bg-white focus:border-blue-500 transition-all" placeholder="输入名称或 SPU 搜索..." value={mainMaterialSearch} onChange={(e) => setMainMaterialSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearchMainMaterial()} />
                     {isSearching && <div className="absolute right-2.5 animate-spin text-blue-500"><Plus size={12} /></div>}
                  </div>
                  <button onClick={handleSearchMainMaterial} className="px-4 h-8 bg-slate-800 text-white rounded font-bold text-[11px] hover:bg-slate-900 transition-all">搜索</button>
               </div>

               {/* 搜索结果面板 */}
               {(foundMaterials.length > 0 || (hasSearched && !isSearching)) && (
                  <div className="mx-2 mt-2 border border-blue-100 bg-white rounded-lg overflow-hidden animate-in fade-in slide-in-from-top-1">
                     <div className="bg-blue-50/50 px-3 py-1.5 border-b border-blue-100 flex justify-between items-center">
                        <span className="text-[10px] font-bold text-blue-600 uppercase">搜索结果 ({foundMaterials.length})</span>
                        <button onClick={handleAddMaterials} disabled={selectedFoundIds.length === 0} className="text-[10px] font-bold bg-blue-600 text-white px-2 py-0.5 rounded">关联所选 ({selectedFoundIds.length})</button>
                     </div>
                        ...
                        <table className="w-full text-left text-[11px]">
                           <tbody className="divide-y divide-slate-50">
                              {foundMaterials.map(m => {
                                 ...
                                 return (
                                   <tr key={m.id} className={`hover:bg-blue-50/30 cursor-pointer ${selectedFoundIds.includes(m.id) ? 'bg-blue-50/50' : ''}`} onClick={() => !isAdded && setSelectedFoundIds(prev => prev.includes(m.id) ? prev.filter(i => i !== m.id) : [...prev, m.id])}>
                                      <td className="px-3 py-2 w-8"><input type="checkbox" disabled={isAdded} checked={selectedFoundIds.includes(m.id)} onChange={() => {}} className="rounded-sm text-blue-600" /></td>
                                      <td className="px-2 py-2 font-bold">{m.name} {isAdded && <span className="text-[9px] bg-emerald-100 text-emerald-600 px-1 rounded-full ml-1">已关联</span>}</td>
                                      <td className="px-2 py-2 text-slate-400 font-mono">{m.spu}</td>
                                      <td className="px-3 py-2 text-right">{!isAdded ? <button onClick={(e) => { e.stopPropagation(); handleSingleAssociate(m); }} className="text-blue-600 font-bold hover:underline">关联</button> : <CheckCircle2 size={14} className="text-emerald-500 ml-auto" />}</td>
                                   </tr>
                                 );
                              })}
                           </tbody>
               ...
               {associatedMaterials.length > 0 && (
                  <div className="mx-2 mt-2 border border-slate-100 rounded-lg overflow-hidden bg-slate-50/20 shadow-sm">
                     <div className="px-3 py-1.5 text-[10px] font-bold text-slate-500 border-b border-slate-100 uppercase tracking-tight">已关联主物料清单</div>
                     <table className="w-full text-left text-[11px] bg-white">
                        <tbody className="divide-y divide-slate-50">
                           {associatedMaterials.map(m => (
                              <tr key={m.id} className="hover:bg-blue-50/20">
                                 <td className="px-3 py-1.5 font-bold text-slate-700">{m.name}</td>
                                 <td className="px-3 py-1.5 text-slate-400 font-mono">{m.spu} / {m.code}</td>
                                 <td className="px-3 py-1.5 text-right"><button onClick={() => setAssociatedMaterials(prev => prev.filter(i => i.id !== m.id))} className="text-slate-300 hover:text-red-500 p-1"><Trash2 size={14} /></button></td>
                              </tr>
                           ))}
                        </tbody>
                     </table>
            ...
            <div ref={sectionRefs['采购信息']} className="space-y-4">
               <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm pb-2 border-b border-slate-100 flex items-center space-x-2 text-slate-800 font-bold text-[13px]"><Calendar size={14} className="text-blue-500" /><span>采购与规格信息</span></div>
               
               <div className="grid grid-cols-2 gap-4 px-2">
                  {/* 分类 A+B */}
                  <div className="space-y-4 p-3 border border-slate-100 rounded-xl bg-slate-50/30 shadow-sm">
                     <div>
                        <div className="flex items-center gap-2 mb-3"><div className="w-1 h-3 bg-blue-500 rounded-full"></div><span className="text-[11px] font-bold text-slate-700">采购基础 Procurement</span></div>
                     ...
                     <div>
                        <div className="flex items-center gap-2 mb-3"><div className="w-1 h-3 bg-emerald-500 rounded-full"></div><span className="text-[11px] font-bold text-slate-700">开票信息 Invoicing</span></div>
                        <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-4 p-3 border border-slate-100 rounded-xl bg-slate-50/30 shadow-sm">
                     <div>
                        <div className="flex items-center gap-2 mb-3"><div className="w-1 h-3 bg-orange-500 rounded-full"></div><span className="text-[11px] font-bold text-slate-700">规格参数 Specifications</span></div>
                     ...
                     <div className="h-px bg-slate-100"></div>
                     <div>
                        <div className="flex items-center gap-2 mb-3"><div className="w-1 h-3 bg-indigo-500 rounded-full"></div><span className="text-[11px] font-bold text-slate-700">产品图片 Images</span></div>
            ...
            <div ref={sectionRefs['首单信息']} className="space-y-3">
               <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm pb-2 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex items-center space-x-2 text-slate-800 font-bold text-[13px]"><Calendar size={14} className="text-blue-500" /><span>首单需求</span></div>
                  <button onClick={() => setFirstOrderRows(prev => [...prev, { id: Date.now(), requester: '', dept: '', team: '', quantity: '', deadline: '' }])} className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 hover:bg-blue-100 transition-all">+ 新增行</button>
               </div>
               <div className="mx-2 border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm">
                  <table className="w-full text-left text-[11px]">
                     ...
                     <tbody className="divide-y divide-slate-50">
                        {firstOrderRows.map(row => (
                           <tr key={row.id} className="hover:bg-slate-50/50">
                              <td className="px-2 py-1"><input className="w-full h-7 bg-transparent px-2 outline-none focus:bg-white border border-transparent focus:border-blue-100 rounded text-[11px]" placeholder="姓名" value={row.requester} onChange={e => updateFirstOrderRow(row.id, 'requester', e.target.value)} /></td>
                              <td className="px-2 py-1">
                                 <div className="flex gap-1">
                                    {/* 部门下拉 */}
                                    <select 
                                       className="w-1/2 h-7 bg-transparent px-1 outline-none focus:bg-white border border-transparent focus:border-blue-100 rounded text-[11px] appearance-none cursor-pointer"
                                       value={row.dept}
                                       onChange={e => updateFirstOrderRow(row.id, 'dept', e.target.value)}
                                    >
                                       <option value="">部门</option>
                                       {Object.keys(MOCK_DEPT_TEAMS).map(dept => <option key={dept} value={dept}>{dept}</option>)}
                                    </select>
                                    {/* Team 下拉 - 联动 */}
                                    <select 
                                       className="w-1/2 h-7 bg-transparent px-1 outline-none focus:bg-white border border-transparent focus:border-blue-100 rounded text-[11px] appearance-none cursor-pointer disabled:cursor-not-allowed disabled:opacity-30"
                                       value={row.team}
                                       disabled={!row.dept}
                                       onChange={e => updateFirstOrderRow(row.id, 'team', e.target.value)}
                                    >
                                       <option value="">Team</option>
                                       {row.dept && MOCK_DEPT_TEAMS[row.dept].map(team => <option key={team} value={team}>{team}</option>)}
                                    </select>
                                 </div>
                              </td>
                              <td className="px-2 py-1 text-center"><input type="number" className="w-full h-7 text-center bg-transparent px-2 outline-none focus:bg-white border border-transparent focus:border-blue-100 rounded text-[11px] font-mono" placeholder="0" value={row.quantity} onChange={e => updateFirstOrderRow(row.id, 'quantity', e.target.value)} /></td>
                              <td className="px-2 py-1 text-center"><input type="date" className="w-full h-7 text-center bg-transparent px-2 outline-none focus:bg-white border border-transparent focus:border-blue-100 rounded text-[11px] text-slate-500" value={row.deadline} onChange={e => updateFirstOrderRow(row.id, 'deadline', e.target.value)} /></td>
                              <td className="px-2 py-1 text-center"><button onClick={() => firstOrderRows.length > 1 && setFirstOrderRows(prev => prev.filter(r => r.id !== row.id))} className="p-1 text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={14} /></button></td>
                           </tr>
                        ))}
                     </tbody>
                  </table>
        ...
        <div className="px-4 py-2.5 border-t border-slate-100 flex justify-end space-x-2 bg-white shrink-0 shadow-inner">
          <button className="px-5 py-1.5 bg-slate-50 text-slate-500 rounded font-bold text-[11px] hover:bg-slate-100 transition-all" onClick={onClose}>取消</button>
          <button className="px-8 py-1.5 bg-blue-600 text-white rounded font-bold text-[11px] hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all active:scale-95">保存配件物料信息</button>
        </div>
      </div>
    </div>
  );
